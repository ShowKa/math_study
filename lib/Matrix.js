...
class Matrix {
    ...
    getEigenValue() {
        ...
        for (let i = 0; i < 20000; i++) {
             ...
             *ウィルキンソンの移動法を行う。
             */
            // 2次元行列を抽出し、その固有値を求める(=対称行列の固有値の推測値)。
            let target = this.rowSize - 1;
            ...
            let twoD = this.subtract(target - 1, 2, target - 1, 2);
            let [evApp1, evApp2] = twoD.getEigenValue2D();
            let u = getCloserValue(mat[target][target], evApp1, evApp2);

            // 2次元行列のうち、右下の要素の値と近い法を近似値として採用する。
            let uI = Matrix.createI(this.rowSize).multiply(u);
    ...
    getEigenValue2D() {
        ...
        let value = b * b - c;
        if (value < 0) throw new Error("Eigen Value is not Real Number");
