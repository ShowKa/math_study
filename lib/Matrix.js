...
class Matrix {
    ...
    /**
     * QR分解
     * Hassenbergに変換させた行列に回転行列をかけ合せる
     * 回転行列は対角要素の直下が0になるように角度を調整する。
     *
     */
    getQR() {
        // to Hassenberg
        let hassenberg = this.getHassenberg();
        ...
        for (let c = 0; c < size - 1; c++) {
            ...
            hassenberg.matrix = q.product(hassenberg).matrix;
    ...
    /**
     * householder法によりHassenberg行列に相似変換した行列を取得。
     */
    getHassenberg() {

        let hassenberg = this.clone();
        ...
        // ....
        for (let c = 0; c < hassenberg.columnSize - 2; c++) {
            // householder変換の対象となるベクトルを抽出。
            // 例えば第一列目の縦ベクトルが (4, 1, -2, 2)の場合、(1, -2, 2)が対象となる。
            // 変換後 => (3, 0, 0)
            let column = hassenberg.getColumn(c);
            column.splice(0, c + 1);
            ...
            hassenberg.matrix = diag.product(hassenberg).product(diag).matrix;
        }
        return hassenberg;
    }
