...
class Matrix {
    ...
    isLowerTriangular() {
        for (let r = 0; r < this.rowSize; r++) {
            for (let c = r + 1; c < this.columnSize; c++) {
                if (this.matrix[r][c] != 0) {
                    return false;
                }
            }
     ...
     * 切り抜き
     */
    subtract(row, rowSize, column, columnSize) {
        let matrix = [];
        for (let r = row; r < row + rowSize; r++) {
            let newRow = [];
            matrix.push(newRow);
            for (let c = column; c < column + columnSize; c++) {
                newRow.push(this.matrix[r][c]);
            }
        }
    ...
    /**
     * QR分解の反復法により固有値を求める。
     * ウィルキンソンの移動法を用いて、収束を早める。
     * @param times QR分解の回数。
     */
    getEigenValue(times = 30000) {
        if (this.rowSize == 2 && this.columnSize == 2) {
            return this.getEigenValue2D();
        }
        ...
        // [補足2]
        // 上三角の固有値は対角成分と一致する。下三角も同様。

        let A = this.clone();

        // 三角行列の場合、対角成分が固有値
        if (A.isLowerTriangular() || A.isUpperTriangular()) {
            let answer = [];
            for (let i = 0; i < A.rowSize; i++) {
                answer.push(A.matrix[i][i]);
            }
            return answer;
        }
        ...
        for (let i = 0; i < times; i++) {
            ...
            // 2 * 2 なら、「固有方程式」を利用して固有値を求める。
            if (A.rowSize == 2) {
                try {
                    let [x, y] = A.getEigenValue2D();
                    answer.push(x);
                    answer.push(y);
                    break;
                } catch (e) {
                    // うまく実数解が求まらない場合、繰り返し処理を継続。
                    // そのうち実数解の求まる形になる。
                }
            }
            ...
            // Hessenberg行列の最も右下の要素の左側 = 0
            // -> 右下の要素 = 固有値に収束したということ。
            let mat = A.matrix;
            let target = A.rowSize - 1;
            if (mat[target][target - 1] == 0) {
                // 左側がすべて0かチェック
                let ok = false;
                ...
                if (ok) {
                    // 最下行と最右列を除去し、次の固有値を求めに行く。
                    answer.push(mat[target][target]);
                    A = A.subtract(0, target, 0, target);
                    continue;
                }
            }
             ...
             * ただし、uはAの固有値の推定値
             *
             * 対角成分の左側にある成分がゼロに収束するに従い、対角成分との差が大きくなります。
             * すると、ゼロにすべき要素がなかなかゼロに収束しにくくなります。
             * よって、対角を予め小さくしておく。（あとで足し合わせれば、相似の関係は崩れない）
             */
            // 右下から2次元行列を抽出し、その固有値を求める(=行列Aの固有値の推定値)。
            let twoD = A.subtract(target - 1, 2, target - 1, 2);
            let u; // 近似値
            try {
                // 2次元行列の右下の要素の値と近い値を近似値として採用する。
                let [evApp1, evApp2] = twoD.getEigenValue2D();
                u = getCloserValue(mat[target][target], evApp1, evApp2);
            } catch (err) {
                // うまく近似値が求まらなかったら0とする。
                // -> uI行列の意味をなくす。
                u = 0;
            ...
            let uI = Matrix.createI(A.rowSize).multiply(u);
            let [Q, R] = A.diff(uI).getQR();
            A = R.product(Q).add(uI);
        };

        if (answer.length != this.rowSize) {
            throw new Error("Couldn't find enough Eigen Values : " + answer);
        }

        return answer;

        function getCloserValue(target, a, b) {
    ...
    /**
     * 2次元行列の場合の固有値を求める。
     * 書きを満たすx(= 固有方程式の解)が固有値。
     * A * vector = x * vector
     * -> det(A - xI) = 0 
     */
    getEigenValue2D() {
        if (this.rowSize != 2 || this.columnSize != 2) throw new Error("not 2D Matrix!!");
        let b = -1 * (this.matrix[0][0] + this.matrix[1][1]) / 2;
        let c = (-1 * this.matrix[0][1] * this.matrix[1][0]) + (this.matrix[0][0] * this.matrix[1][1]);
        let value = b * b - c;
        if (value < 0) {
            throw new Error("Eigen Values are not Real Number");
        }
        return [-1 * b - Math.sqrt(value), -1 * b + Math.sqrt(value)];
    }
    ...
    getQR() {
        ...
        if (this.isUpperTriangular()) {
            return [Matrix.createI(this.rowSize), this];
        }
    ...
    getHessenberg() {
        ...
        for (let c = 0; c < hessenberg.columnSize - 2; c++) {
            ...
            // すでにhouseholdで加工済みの場合処理は不要。
            let next = true;
            for (let _v in column) {
                if (_v == 0) continue;
                if (column[_v] != 0) {
                    next = false;
                    break;
